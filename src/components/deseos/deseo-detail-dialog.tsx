...
import { quitarDeseo } from "@/lib/firestore/deseos";
...
interface DeseoDetailDialogProps {
  isbn: string | null;
  global?: LibroGlobal;
  onClose: () => void;
}

export function DeseoDetailDialog({
  isbn,
  global,
  onClose,
}: DeseoDetailDialogProps) {
  const { user } = useAuth();
  ...
  async function handleGuardarEdicion() {
    ...
    if (!formEdit.titulo.trim()) {
      toast.error(t("deseoDetail.errorTituloObligatorio"));
      return;
    }
    ...
    try {
      ...
      toast.success(t("deseoDetail.datosActualizados"));
      setEditando(false);
    } catch (err) {
      logError("Error editando el libro:", err);
      toast.error(t("deseoDetail.errorGuardandoCambios"));
    } finally {
  ...
  async function handlePublicarResena() {
    if (!isbn || !user) return;
    if (!comentario.trim()) {
      toast.error(t("deseoDetail.errorEscribirComentario"));
      return;
    }
    try {
      await publicarResena(
        isbn,
        user.uid,
        user.displayName ?? user.email ?? t("deseoDetail.anonimo"),
        estrellas,
        comentario.trim()
      );
      setComentario("");
      ...
      toast.error(t("deseoDetail.errorPublicandoResena"));
    }
  }

  async function handleQuitar() {
    if (!isbn || !user) return;
    const titulo = global?.titulo ?? t("deseoDetail.esteLibro");
    if (!window.confirm(t("deseoDetail.confirmarQuitar", { titulo }))) {
      return;
    }
    try {
      await quitarDeseo(user.uid, isbn);
      onClose();
    } catch (err) {
      logError("Error quitando de deseados:", err);
      toast.error(t("deseoDetail.errorQuitando"));
    }
  }
        ...
        <div className="flex flex-col gap-4 text-sm">
          {editando ? (
            <div className="flex flex-col gap-3">
              ...
              />
              <FieldEdit label={t("deseoDetail.campoTitulo")}>
                <Input
                  value={formEdit.titulo}
                  onChange={(e) => setCampoEdit("titulo", e.target.value)}
                />
              </FieldEdit>
              <FieldEdit label={t("deseoDetail.campoSubtitulo")}>
                <Input
                  value={formEdit.subtitulo}
                  onChange={(e) => setCampoEdit("subtitulo", e.target.value)}
                />
              </FieldEdit>
              <FieldEdit label={t("deseoDetail.campoAutor")}>
                <Input
              ...
              <FieldEdit label={t("deseoDetail.campoIlustrador")}>
                <Input
              ...
              <div className="flex gap-2">
                <FieldEdit label={t("deseoDetail.campoEditorial")} className="flex-[2]">
                  <Input
                ...
                <FieldEdit label={t("deseoDetail.campoAnio")} className="w-20">
                  <Input
                    value={formEdit.anio}
                    onChange={(e) => setCampoEdit("anio", e.target.value)}
                  />
                </FieldEdit>
                <FieldEdit label={t("deseoDetail.campoPaginas")} className="w-20">
                  <Input
                    value={formEdit.paginas}
                    onChange={(e) => setCampoEdit("paginas", e.target.value)}
                  />
                </FieldEdit>
                <FieldEdit label={t("deseoDetail.campoVolumen")} className="w-20">
                  <Input
                    placeholder={t("deseoDetail.tomoPlaceholder")}
              ...
              <div className="flex gap-2">
                <FieldEdit label={t("deseoDetail.campoGenero")} className="flex-1">
                  <GeneroSelect
                    value={formEdit.genero}
                    onValueChange={(v) => setCampoEdit("genero", v)}
                  />
                </FieldEdit>
                <FieldEdit label={t("deseoDetail.campoIdioma")} className="w-36">
                  <IdiomaSelect
              ...
              <FieldEdit label={t("deseoDetail.campoSinopsis")}>
                <Textarea
                  rows={3}
                ...
                <div>
                  ...
                  {global?.ilustrador && (
                    <div className="text-xs text-muted-foreground">
                      {t("deseoDetail.ilustradoPor", { ilustrador: global.ilustrador })}
                    </div>
                  )}
                  <div className="mt-1 text-xs text-muted-foreground">
                    {t("deseoDetail.ratingInfo", {
          ...
          <div className="border-t pt-3">
            <div className="mb-2 flex items-center justify-between">
              <div className="font-semibold">{t("deseoDetail.resenasComunidad")}</div>
              <button
                onClick={() => setReviewOpen((o) => !o)}
                className="text-xs font-semibold text-primary underline"
              >
                {reviewOpen ? t("deseoDetail.cancelar") : t("deseoDetail.escribirResena")}
              </button>
            </div>
            ...
            {reviewOpen && (
              <div className="mb-3 flex flex-col gap-2 rounded-lg border p-3">
                ...
                <Textarea
                  placeholder={t("deseoDetail.placeholderResena")}
                  value={comentario}
                  onChange={(e) => setComentario(e.target.value)}
                  rows={3}
                />
                <Button size="sm" className="self-end" onClick={handlePublicarResena}>
                  {t("deseoDetail.publicarResena")}
                </Button>
              </div>
            )}
